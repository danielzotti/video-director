...
@Component({
  selector: 'app-crop-position-controls',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, UiButtonComponent],
  templateUrl: './crop-position-controls.component.html',
  styleUrl: './crop-position-controls.component.scss',
})
export class CropPositionControlsComponent {
  idPrefix = input<string>('crop');
  offsetX = input<string>('-50');
  offsetY = input<string>('-50');
  zoom = input<string>('1');

  anchorSelected = output<CropAnchor>();
  offsetXChanged = output<number>();
  offsetYChanged = output<number>();
  zoomChanged = output<number>();

  protected readonly ANCHOR_OFFSET_MAP: Record<CropAnchor, { x: number; y: number }> = {
    'top-left': { x: 0, y: 0 },
    'top-center': { x: -50, y: 0 },
    'top-right': { x: -100, y: 0 },
  ...
  protected selectAnchor(anchor: CropAnchor): void {
    this.anchorSelected.emit(anchor);
  }

  protected isAnchorActive(anchor: CropAnchor): boolean {
